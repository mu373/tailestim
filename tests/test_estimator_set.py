...
def test_tail_estimator_set_fit():
    ...
    assert (
        estimator_set.ordered_data[0] >= estimator_set.ordered_data[-1]
    )  # Check ordering
...
def test_tail_estimator_set_errors():
    ...
    with pytest.raises(
        ValueError, match="Diagnostic plots require bootstrap to be enabled"
    ):
        estimator_set.plot_diagnostics()
...
def test_tail_estimator_set_parameters():
    ...
    custom_alpha = 0.7
    estimator_set = TailEstimatorSet(
        number_of_bins=custom_bins, r_smooth=custom_r_smooth, alpha=custom_alpha
    )
