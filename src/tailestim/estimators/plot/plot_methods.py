...
def make_plots(
    ...
    # Smooth Hill results
    k_sh_arr, xi_sh_arr = (
        results["smooth_hill"]["k_arr_"],
        results["smooth_hill"]["xi_arr_"],
    )
    ...
    if bootstrap_flag and "k_star_" in results["hill"]:
        ...
        max_h_index1 = results["hill"]["bootstrap_results_"]["first_bootstrap_"][
            "max_index_"
        ]
        ...
        max_h_index2 = results["hill"]["bootstrap_results_"]["second_bootstrap_"][
            "max_index_"
        ]
    else:
        k_h_star = None
    ...
    if bootstrap_flag and "k_star_" in results["moments"]:
        ...
        x1_m_arr = results["moments"]["bootstrap_results_"]["first_bootstrap_"][
            "x_arr_"
        ]
        n1_m_amse = results["moments"]["bootstrap_results_"]["first_bootstrap_"][
            "amse_"
        ]
        k1_m = results["moments"]["bootstrap_results_"]["first_bootstrap_"]["k_min_"]
        max_m_index1 = results["moments"]["bootstrap_results_"]["first_bootstrap_"][
            "max_index_"
        ]
        x2_m_arr = results["moments"]["bootstrap_results_"]["second_bootstrap_"][
            "x_arr_"
        ]
        n2_m_amse = results["moments"]["bootstrap_results_"]["second_bootstrap_"][
            "amse_"
        ]
        k2_m = results["moments"]["bootstrap_results_"]["second_bootstrap_"]["k_min_"]
        max_m_index2 = results["moments"]["bootstrap_results_"]["second_bootstrap_"][
            "max_index_"
        ]
    else:
        k_m_star = None
    ...
    if bootstrap_flag and "k_star_" in results["kernel"]:
        ...
        max_k_index1 = results["kernel"]["bootstrap_results_"]["first_bootstrap_"][
            "max_index_"
        ]
        x2_k_arr = results["kernel"]["bootstrap_results_"]["second_bootstrap_"][
            "x_arr_"
        ]
        n2_k_amse = results["kernel"]["bootstrap_results_"]["second_bootstrap_"][
            "amse_"
        ]
        h2 = results["kernel"]["bootstrap_results_"]["second_bootstrap_"]["h_min"]
        max_k_index2 = results["kernel"]["bootstrap_results_"]["second_bootstrap_"][
            "max_index_"
        ]
    ...
    if savedata:
        # Save Pickands data
        with open(
            os.path.join(output_dir + "/" + output_name + "_pickands.dat"), "w"
        ) as f:
        ...
        with open(
            os.path.join(output_dir + "/" + output_name + "_sm_hill.dat"), "w"
        ) as f:
        ...
        if bootstrap_flag and k_h_star is not None:
            with open(
                os.path.join(output_dir + "/" + output_name + "_adj_hill_estimate.dat"),
                "w",
            ) as f:
    ...
    if bootstrap_flag and k_h_star is not None:
        ...
        if len(x) > 0:  # Make sure we have points to plot
            ...
            if xi_h_star > 0:
                axes[0, 1].plot(
                    x,
                    [l * (float(xmin) / k) ** alpha for k in x],
                    color="#fb8072",
                    ls="--",
                    lw=2,
                ...
                axes[0, 1].plot(
                    (x[-1]),
                    [l * (float(xmin) / x[-1]) ** (alpha)],
                    color="#fb8072",
                    ls="none",
                    marker="o",
                    markerfacecolor="none",
                    markeredgecolor="#fb8072",
                    markeredgewidth=3,
                    markersize=10,
                )
    ...
    axes[1, 0].plot(
        k_sh_arr[indices_to_plot_sh],
        xi_sh_arr[indices_to_plot_sh],
        color="#b3de69",
        alpha=0.8,
        label="Smooth Hill",
        zorder=10,
    ...
    axes[1, 0].plot(
        k_h_arr[indices_to_plot_h],
        xi_h_arr[indices_to_plot_h],
        color="#fb8072",
        alpha=0.8,
        label="Adjusted Hill",
        zorder=10,
...
def make_diagnostic_plots(
    ...
    if n1_h_amse is not None and x1_h_arr is not None:
        n1_h_amse[np.where((n1_h_amse == np.inf) | (n1_h_amse == -np.inf))] = np.nan
        axes_d[0].set_ylim((0.1 * np.nanmin(n1_h_amse[min_k1:max_k1]), 1.0))

    axes_d[0].set_xlabel("Fraction of Bootstrap Order Statistics", fontsize=20)
    ...
    if x1_h_arr is not None and n1_h_amse is not None and k1_h is not None:
        axes_d[0].plot(
            x1_h_arr[min_k1:max_k1],
            n1_h_amse[min_k1:max_k1],
            alpha=0.5,
            lw=1.5,
            color="#d55e00",
            label=r"$n_1$ samples",
        ...
        axes_d[0].scatter(
            [k1_h],
            [n1_h_amse[int(len(x1_h_arr) * k1_h) - 1]],
            color="#d55e00",
            marker="o",
            edgecolor="black",
            alpha=0.5,
    ...
    if x2_h_arr is not None and n2_h_amse is not None and k2_h is not None:
        axes_d[0].plot(
            x2_h_arr[min_k2:max_k2],
            n2_h_amse[min_k2:max_k2],
            alpha=0.5,
            lw=1.5,
            color="#0072b2",
            label=r"$n_2$ samples",
        ...
        axes_d[0].scatter(
            [k2_h],
            [n2_h_amse[int(len(x2_h_arr) * k2_h) - 1]],
            color="#0072b2",
            marker="o",
            edgecolor="black",
            alpha=0.5,
    ...
    if max_h_index1 is not None and x1_h_arr is not None:
        axes_d[0].axvline(
            max_h_index1 / float(len(x1_h_arr)),
            color="#d55e00",
            ls="--",
            alpha=0.5,
    ...
    if max_h_index2 is not None and x2_h_arr is not None:
        axes_d[0].axvline(
            max_h_index2 / float(len(x2_h_arr)),
            color="#0072b2",
            ls="--",
            alpha=0.5,
    ...
    if savedata and output_file_path is not None:
        with open(
            os.path.join(output_dir + "/" + output_name + "_adjhill_diagn1.dat"), "w"
        ) as f:
        ...
        with open(
            os.path.join(output_dir + "/" + output_name + "_adjhill_diagn2.dat"), "w"
        ) as f:
    ...
    n1_m_amse[np.where((n1_m_amse == np.inf) | (n1_m_amse == -np.inf))] = np.nan
    axes_d[1].set_yscale("log")
    axes_d[1].set_ylim((0.1 * np.nanmin(n1_m_amse[min_k1:max_k1]), 1.0))
    axes_d[1].set_xlabel("Fraction of Bootstrap Order Statistics", fontsize=20)
    ...
    axes_d[1].plot(
        x1_m_arr[min_k1:max_k1],
        n1_m_amse[min_k1:max_k1],
        alpha=0.5,
        lw=1.5,
        color="#d55e00",
        label=r"$n_1$ samples",
    ...
    axes_d[1].scatter(
        [k1_m],
        [n1_m_amse[int(len(x1_m_arr) * k1_m) - 1]],
        color="#d55e00",
        marker="o",
        edgecolor="black",
        alpha=0.5,
    ...
    axes_d[1].plot(
        x2_m_arr[min_k2:max_k2],
        n2_m_amse[min_k2:max_k2],
        alpha=0.5,
        lw=1.5,
        color="#0072b2",
        label=r"$n_2$ samples",
    ...
    axes_d[1].scatter(
        [k2_m],
        [n2_m_amse[int(len(x2_m_arr) * k2_m) - 1]],
        color="#0072b2",
        marker="o",
        edgecolor="black",
        alpha=0.5,
    ...
    axes_d[1].axvline(
        max_m_index1 / float(len(x1_m_arr)),
        color="#d55e00",
        ls="--",
        alpha=0.5,
        label=r"Minimization boundary for $n_1$ sample",
    ...
    axes_d[1].axvline(
        max_m_index2 / float(len(x2_m_arr)),
        color="#0072b2",
        ls="--",
        alpha=0.5,
        label=r"Minimization boundary for $n_2$ sample",
    ...
    if savedata and output_file_path is not None:
        ...
        with open(
            os.path.join(output_dir + "/" + output_name + "_mom_diagn_points.dat"), "w"
        ) as f:
            f.write(
                "Min for n1 sample: "
                + str(k1_m)
    ...
    axes_d[2].scatter(
        [h1],
        [n1_k_amse[np.where(x1_k_arr == h1)][0]],
        color="#d55e00",
        marker="o",
        edgecolor="black",
        alpha=0.5,
    ...
    axes_d[2].scatter(
        [h2],
        [n2_k_amse[np.where(x2_k_arr == h2)][0]],
        color="#0072b2",
        marker="o",
        edgecolor="black",
        alpha=0.5,
    ...
    if savedata and output_file_path is not None:
        ...
        with open(
            os.path.join(output_dir + "/" + output_name + "_kern_diagn_points.dat"), "w"
        ) as f:
            f.write(
                "Min for n1 sample: "
                + str(h1)
    ...
    if output_file_path is not None:
        diag_plots_path = (
            os.path.dirname(os.path.realpath(output_file_path))
            + "/"
